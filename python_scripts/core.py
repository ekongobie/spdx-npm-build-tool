...
# import spdx.utils as spdxutils
# import spdx.version
# import spdx.package
...
class SPDXFile(object):
    ...
    #     ext_doc_ref = document.ExternalDocumentRef(
    #         self.code_extra_params["ext_doc_ref"],
    #         self.code_extra_params["tool_version"],
    #         checksum.Algorithm("SHA1", utils.get_file_hash(self.full_file_path or "")),
    #     )
    ...
    #         )
    #
    #     package.check_sum = checksum.Algorithm("SHA1", self.get_package_checksum())
    #
    #     package.homepage = spdxutils.SPDXNone()
    #     package.verif_code = self.get_package_verification_code()
    #
    #     package.source_info = spdxutils.SPDXNone()
    #     package.conc_lics = spdxutils.NoAssert()
    #
    #     package.license_declared = spdxutils.NoAssert()
    #     package.cr_text = spdxutils.SPDXNone()
    #
    ...
    #     self.spdx_document = document.Document(
    #         version=version.Version(2, 1),
    #         data_license=document.License.from_identifier(
    ...
    #
    #     package = self.spdx_document.package = package.Package(
    #         download_location=spdxutils.NoAssert(), version=self.get_package_version()
    #     )
    ...
    #                     name=name,
    #                     chk_sum=checksum.Algorithm(
    #                         "SHA1", utils.get_file_hash(file_data["FileName"]) or ""
    #                     ),
    #                 )
    #                 spdx_license = None
    #                 if self.doc_type == utils.TAG_VALUE:
    #                     spdx_license = document.License.from_identifier(
    #                         file_data["SPDXID"]
    #                     )
    #                 else:
    ...
    #                         spdx_license = document.ExtractedLicense(licenseref_id)
    #                     spdx_license.name = spdxutils.NoAssert()
    #                     comment = "N/A"
    #                     spdx_license.comment = comment
    #                     text = spdxutils.NoAssert()
    #                     if not text:
    #                         text = comment
    ...
    #                 file_entry.conc_lics = spdxutils.NoAssert()
    #                 file_entry.copyright = spdxutils.SPDXNone()
